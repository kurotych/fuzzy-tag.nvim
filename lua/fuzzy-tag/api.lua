...
M.add_tag = function(file_path, tag_name)
	...
	L.sqlite_db:with_open(function(db)
		local tag_id = get_or_insert_id(function()
			return db.tags:get({ where = { name = tag_name_l } })
		end, function()
			return db.tags:insert({ name = tag_name_l })
		end)
		local file_id = get_or_insert_id(function()
			return db.files:get({ where = { file_path = file_path } })
		end, function()
			return db.files:insert({ file_path = file_path })
		end)
